...
void set_desired_inferior(int use_general)
{
    ...
    if (use_general == 1)
    {
        found = (struct thread_info *)find_inferior_id(&all_threads, general_thread);
    }
    else
    {
        found = NULL;
       ...
       signal to.  */
        ...
        {
            found = (struct thread_info *)find_inferior_id(&all_threads, step_thread);
        }

        if (found == NULL)
        {
            found = (struct thread_info *)find_inferior_id(&all_threads, cont_thread);
        }
    }
