...
static task_t darwin_task_for_pid(pid_t pid)
{
    task_t task;
    kern_return_t kr;
...
static thread_act_t darwin_get_first_thread(task_t task)
{
...
static int darwin_create_inferior(char *program, char **allargs)
{
    pid_t pid;
...
static int darwin_attach(int pid)
{
    struct darwin_process_info *proc;
    int ret;
...
static void darwin_detach(void)
{
    struct darwin_process_info *proc;
...
static int darwin_thread_alive(int pid)
{
...
/* Resume the inferior.  */
static void darwin_resume(struct thread_resume *resume_info)
{
    struct darwin_process_info *proc;
    int step = 0;
    int sig = 0;
...
static unsigned char darwin_wait(char *status)
{
    int w;
    int ret;
    struct darwin_process_info *proc;
...
static void darwin_fetch_registers(int regno)
{
    struct darwin_process_info *proc;
    proc = get_thread_process(current_inferior);
...
/* Store registers using Mach thread_set_state.
   Delegates to the arch-specific low target.  */
static void darwin_store_registers(int regno)
{
    struct darwin_process_info *proc;
    proc = get_thread_process(current_inferior);
...
static void darwin_read_memory(CORE_ADDR memaddr, char *myaddr, int len)
{
    kern_return_t kr;
    vm_offset_t data;
    ...
    kr = vm_read(current_task, (vm_address_t)memaddr, len, &data, &data_count);
    if (kr != KERN_SUCCESS)
    {
        ...
        for (i = 0; i < len; i += sizeof(int))
        {
            ...
            errno = 0;
            word = ptrace(PT_READ_D, proc->pid, (caddr_t)(unsigned long)(memaddr + i), 0);
            if (errno != 0)
            {
...
static int darwin_write_memory(CORE_ADDR memaddr, const char *myaddr, int len)
{
    kern_return_t kr;
    ...
    kr = vm_write(current_task, (vm_address_t)memaddr, (vm_offset_t)myaddr, len);
    if (kr != KERN_SUCCESS)
    {
        /* The memory might not be writable. Try to change protection.  */
        kr = vm_protect(current_task, (vm_address_t)memaddr, len, FALSE,
    ...
    if (kr != KERN_SUCCESS)
    {
        ...
        for (i = 0; i < len; i += sizeof(int))
        {
            ...
            errno = 0;
            ptrace(PT_WRITE_D, proc->pid, (caddr_t)(unsigned long)(memaddr + i), word);
            if (errno != 0)
            {
                return errno;
            }
...
static void darwin_send_signal(int signum)
{
    struct darwin_process_info *proc;
    proc = get_thread_process(current_inferior);
...
void initialize_low(void)
{
    ...
    set_breakpoint_data(the_low_target.breakpoint, the_low_target.breakpoint_len);
    init_registers();
