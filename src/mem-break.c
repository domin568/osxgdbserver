...
void set_breakpoint_at(CORE_ADDR where, void (*handler)(CORE_ADDR))
{
    ...
    (*the_target->read_memory)(where, bp->old_data, breakpoint_len);
    (*the_target->write_memory)(where, breakpoint_data, breakpoint_len);
...
static void delete_breakpoint(struct breakpoint *bp)
{
    ...
    {
        breakpoints = bp->next;
        (*the_target->write_memory)(bp->pc, bp->old_data, breakpoint_len);
        free(bp);
        return;
    }
    cur = breakpoints;
...
static struct breakpoint *find_breakpoint_at(CORE_ADDR where)
{
...
static void reinsert_breakpoint_handler(CORE_ADDR stop_pc)
{
    ...
    (*the_target->write_memory)(orig_bp->pc, breakpoint_data, breakpoint_len);
...
void reinsert_breakpoint_by_bp(CORE_ADDR stop_pc, CORE_ADDR stop_at)
{
    ...
    (*the_target->write_memory)(orig_bp->pc, orig_bp->old_data, breakpoint_len);
    orig_bp->reinserting = 1;
}

void uninsert_breakpoint(CORE_ADDR stopped_at)
{
    struct breakpoint *bp;
...
void reinsert_breakpoint(CORE_ADDR stopped_at)
{
    struct breakpoint *bp;
    ...
    (*the_target->write_memory)(bp->pc, breakpoint_data, breakpoint_len);
    bp->reinserting = 0;
}
