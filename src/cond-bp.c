...
static int parse_reg_name(const char *name)
{
    ...
    if (strcmp(upper, "PC") == 0)
    {
        return 64;
    }
    if (strcmp(upper, "PS") == 0)
    {
        return 65;
    }
    if (strcmp(upper, "CR") == 0)
    {
        return 66;
    }
    if (strcmp(upper, "LR") == 0)
    {
        return 67;
    }
    if (strcmp(upper, "CTR") == 0)
    {
        return 68;
    }
    if (strcmp(upper, "XER") == 0)
    {
        return 69;
    }
...
static int compare_values(unsigned long current, int op, unsigned long expected)
{
    switch (op)
    {
        case COND_OP_EQ:
...
static void cmd_cond_bp(const char *args, char *own_buf)
{
    ...
    {
        make_response(own_buf, "Error: usage:\n"
                               "  cond_bp <addr> str <reg+off> ==|!= <string>\n"
                               "  cond_bp <addr> reg <reg_name> <op> <hex_value>\n"
                               "  cond_bp <addr> mem <hex_addr> <size> <op> <hex_value>\n");
        return;
    ...
    if (strcmp(type_str, "str") == 0)
    {
        ...
        {
            make_response(own_buf, "Error: usage: cond_bp <addr> str <expr> ==|!= <string>\n");
            return;
        }
        ...
        {
            make_response(own_buf, "Error: string conditions only support == and !=\n");
            return;
        }
       ...
       Skip 4 tokens: addr, type, expr, op. */
        {
            ...
            i = strlen(target) - 1;
            while (i >= 0 && (target[i] == ' ' || target[i] == '\t' || target[i] == '\r' ||
                              target[i] == '\n'))
            {
                target[i--] = '\0';
            }
        ...
        fprintf(stderr, "cond_bp[%d]: 0x%lx str reg=%d off=%d op=%s target=\"%s\"\n", slot, bp_addr,
                reg_index, offset, op_str, target);
    }
    ...
    else if (strcmp(type_str, "mem") == 0)
    {
        ...
        if (sscanf(args, "%*s %*s %31s %d %3s %31s", maddr_str, &mem_size, op_str, val_str) < 4)
        {
            make_response(own_buf, "Error: usage: cond_bp <addr> mem <hex_addr> "
                                   "<size> <op> <hex_value>\n");
            return;
        }
...
static void cmd_remove_cond_bp(const char *args, char *own_buf)
{
    unsigned long addr;
    int i, found = 0;
    char msg[128];
...
static void cmd_help(char *own_buf)
{
    make_response(own_buf, "Monitor commands:\n"
        ...
        else if (cb->cond_type == COND_TYPE_MEM)
        {
            ...
            switch (cb->mem_size)
            {
                ...
                case 4:
                    current = ((unsigned long)mem_buf[0] << 24) |
                              ((unsigned long)mem_buf[1] << 16) | ((unsigned long)mem_buf[2] << 8) |
                              mem_buf[3];
                    break;
            }
            ...
            if (result)
            {
                fprintf(stderr, "cond_bp HIT: 0x%lx mem[0x%lx:%d]=0x%lx\n", (unsigned long)pc,
                        (unsigned long)cb->mem_addr, cb->mem_size, current);
            }
            return result ? 1 : 2;
        }
    }
