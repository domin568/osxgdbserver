...
char *target_signal_to_string(enum target_signal sig)
{
...
{
    if ((sig >= TARGET_SIGNAL_FIRST) && (sig <= TARGET_SIGNAL_LAST) && signals[sig].name != NULL)
    {
...
enum target_signal target_signal_from_name(char *name)
{
    ...
    for (sig = TARGET_SIGNAL_HUP; sig < TARGET_SIGNAL_LAST;
